...
export class Room extends EventEmitter {
  ...
  async mint(_finalMetadata) {
    ...
    try {
      ...
      const collectionID = await this._getUserCollection(
        _publicAddress,
        contractAddress
      );

      // Create the blob and formData for the token
      ...
      const response = await fetch(
        "https://preserve.nft.storage/api/v1/collection/add_tokens",
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${process.env.REACT_APP_NFTSTORAGE_TOKEN}`,
          },
          body: formData,
        }
      );
 ...
 */
  ...
  // Add this new method
  async _getUserCollection(_publicAddress, contractAddress) {
    try {
      // Try to get existing user first
      const user = await getUser(_publicAddress);

      // If user has a collection ID stored, use that
      if (user.nftCollectionId) {
        return user.nftCollectionId;
      }
      ...
      const createCollectionResponse = await fetch(
        "https://preserve.nft.storage/api/v1/collection/create_collection",
        {
          ...
          body: JSON.stringify({
            ...
            collectionName: `Ambr Collection - ${_publicAddress}`,
      ...
      if (!createCollectionResponse.ok) {
        throw new Error(
          `Failed to create collection: ${createCollectionResponse.statusText}`
        );
      }

      const { collectionID } = await createCollectionResponse.json();

      // Store the collection ID with the user
      ...
      console.error("Error managing user collection:", error);
      throw error;
    }
  }
}

...
export class Room extends EventEmitter {
  ...
  async mint(_finalMetadata) {
    ...
    try {
      ...
      const gasCost = ethers.utils.formatUnits(gasPrice * estGas, "gwei");
      console.log("Gas Cost (gwei):", gasCost.toString());

      // Send transaction to mint NFT with CID
      const tx = await contract.issue(_publicAddress, cid, {
        //gasPrice: ethers.utils.parseUnits(gasPrice.toString(), "gwei"),
        gasPrice: ethers.utils.parseUnits(gasCost.toString(), "gwei"),
        gasLimit: 30000000,
      });

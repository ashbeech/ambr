...
export class Room extends EventEmitter {
  ...
  async mint(_finalMetadata) {
    ...
    const randomAmbr = `/images/amber-${Math.floor(Math.random() * 7) + 1}.png`;
    ...
    const jsonDataPUBLIC = JSON.stringify({
      ...
      description:
        "This token contains a permentant, immutable, and encrypted record of a file shared via Ambr.link.",
      ...
      attributes: [
        {
          trait_type: "Type",
          value: "Ambr Secured File Share [End-to-End Encrypted]",
        },
      ],
    });

    /* 
    const rndAmbr = await fetch(randomAmbr, {
      method: "GET",
    })
      .then((res) => {
        return res.blob();
    ...
    */
    //const rAmbr = new File([rndAmbr], "ambr.png", meta_data);
    ...
    if (
      _form_values.mode ||
      (emailto[0] !== undefined && isValidEmail(emailto[0]))
    ) {
      //console.log("Emal to: ", emailto);
      //console.log("Emal to length: ", emailto.length);
    ...
    // TODO: Emit minting complete as event

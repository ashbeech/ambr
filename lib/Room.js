...
export class Room extends EventEmitter {
  ...
  async mint(_finalMetadata) {
    ...
    const _email = _finalMetadata.email;
    const hash = roomMintMeta.shasum ? roomMintMeta.shasum : null;

    console.log("Email from:", _email);
    ...
    const jsonDataPUBLIC = JSON.stringify({
      ...
      description: `The contents of this metadata and its corresponding file are confidential in nature. The unauthorised misuse, reproduction, distribution, or theft of the work within is strictly prohibited. All rights, including intellectual property, belong to the content's original ${
        _form_values.creators !== null && _form_values.creators.length > 1
          ? "creators"
          : "creator"
      }.`,
      ...
      const response = await fetch(
        "https://api.pinata.cloud/pinning/pinJSONToIPFS",
        {
          ...
          headers: {
            ...
            Authorization: `Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..ek18bA7U44l7xqzqs5jWjn8Cb1esA5rySRwWKs1RgIU`,
          },
          body: JSON.stringify({
            pinataContent: jsonDataPUBLIC,
            pinataMetadata: {
              name: `Ambr File ${roomMintMeta.id}`,
            },
            pinataOptions: {
    ...
    if (
      ...
      async function sendEmail(to, from, id, link) {
        if (!to || !id) return;
        try {
      ...
      await sendEmail(emailto, _email, roomMintMeta.id, url, _publicAddress);
    }
    ...
    if (_publicAddress && this.mintState === "Sealed") {
      // Convert publicAddress to hash to match the record obscured at rest
      const idHash = makeHash(_publicAddress);
      if (idHash) {
        try {
   ...
   * The files' fingerprint hash (SHA256) is then calculated and saved. The function then generates a reader token and salt, which are used to authenticate the client and access the room.
  ...
  async create(uploadFiles) {
    ...
    */
    ...
    // If shasum is provided, continue with the processing
    if (shasum) {
      ...
      // Update the room configuration with the shasum, encryptedTorrentFile, and file size information
      ...
      // If sha checksum is not provided, set cloudState to Upload Failed
      this.cloudState = "Upload Failed";
      // Log error message
      console.error("No sha checksum provided");
    }
      ...
      if (room != null && cid != null) {
        async function getFileFromIPFS(_url) {
          try {
            ...
            console.error("IPFS: Unable to fetch url", err);
          }
        }
        /* 
        console.log("cid: ", cid);
        ...
        */
        try {
          const IPFSdata = await getFileFromIPFS(
            `https://${cid}.ipfs.${ipfsGateway}`
          );

          if (
            !IPFSdata ||
            !IPFSdata.encryptedData ||
            !IPFSdata.encryptedData.data
          ) {
  ...
  async verify(uploadFiles, sha, id, key, idHash) {
    this.verifyState = "Verifying";
    this.emit("verifyState", this.verifyState);

    //console.log("VERIFYING ROOM: ", sha, id, key, idHash);
    // Hash each file and increment the file counter
    const numberOfFiles = uploadFiles.length;
    let fileCounter = 0;
    let shasum = "";
    const is256 = sha.length > 60 ? true : false;
    ...
    for (const file of uploadFiles) {
      ...
      await hashFile(
        ...
        (hashedFile, hash) => {
          ...
          fileCounter++;
          // Append the hash to the SHA sum
          shasum += hash;
          ...
          if (fileCounter === numberOfFiles) {
            // Compare the calculated SHA sum with the provided SHA
            if (sha !== shasum) {

...
export class Room extends EventEmitter {
  ...
  async mint(_finalMetadata) {
    ...
    console.log("_finalMetadata: ", _finalMetadata);
    ...
    //const remainingDownloads = defaultMaxRoomDownloads;
    //const expiresAtTimestampMs = roomMintMeta.expiresAtTimestampMs;
    ...
    try {
      ...
      const estGas = await contract.estimateGas.issue(_publicAddress, cid);
      console.log("Estimated Cost:", estGas.toString());

      console.log(
        "Gas Cost (format wei):",
        ethers.utils.formatUnits(estGas, "wei")
      );
      console.log(
        "Gas Cost (format gwei):",
        ethers.utils.formatUnits(estGas, "gwei")
      );
      console.log(
        "Gas Cost (parse wei):",
        ethers.utils.parseUnits(estGas, "wei")
      );
      console.log(
        "Gas Cost (parse gwei):",
        ethers.utils.parseUnits(estGas, "gwei")
      ); */

      /*
      // Send transaction to mint NFT with CID
      const tx = await contract.issue(_publicAddress, cid, {
 ...
 */
      ...
      const gasPrice = await provider.getGasPrice();
      const gasUnits = await contract.estimateGas.issue(_publicAddress, cid);
      const transactionFee = ethers.BigNumber.from(
        gasPrice.mul(gasUnits) / 100000000000000
      );
      console.log("transactionFee in wei: " + transactionFee.toString());
      console.log(
        "transactionFee in gwei: " +
          ethers.utils.formatUnits(transactionFee, "gwei")
      );
      console.log("txFee: ", transactionFee.toString());
      console.log(
        "OUTCOME gasPrice: ",
        ethers.utils.parseUnits(transactionFee.toString(), "gwei")
      );
      console.log("Raw transactionFee: ", transactionFee.toString());

      // Send transaction to mint NFT with CID
      const tx = await contract.issue(_publicAddress, cid, {
        //gasPrice: ethers.utils.parseUnits("30", "gwei"),
        //gasPrice: ethers.BigNumber.from(transactionFee.toString()),
        //gasPrice: transactionFee.toString(),
        gasPrice: ethers.utils.parseUnits(transactionFee.toString(), "gwei"),
        gasLimit: 3000000,
      });

...
export class Room extends EventEmitter {
  ...
  async mint(_finalMetadata) {
    ...
    try {
      const metadataBlob = new Blob([jsonDataPUBLIC]);

      // Upload metadata to NFT.Storage and get the CID
      const cid = await NFTStorage.upload(metadataBlob);
      console.log("Metadata CID:", cid);

      // Create CSV content with proper headers and data
      const csvContent =
        "tokenID,cid\n" + // Header row
        `${roomMintMeta.id},${cid}`; // Data row

      // Create the CSV file blob
      const csvBlob = new Blob([csvContent], { type: "text/csv" });

      // Create FormData with collection ID and CSV file
      const formData = new FormData();
      formData.append("collectionID", "bf656d07-eb29-4a08-ad92-b9eed4b671fa");
      ...
      const { cid: responseCid } = await response.json();

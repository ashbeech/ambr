  ...
  async join(id, key) {
    ...
    while (true) {
      ...
      if (room != null && cid != null) {
          ...
          for (const url of gateways) {
            try {
              ...
              try {
                ...
                continue; // response wasn't valid JSON, try next gateway
              }
            } catch (err) {
        ...
        try {
          const IPFSdata = await getFileFromIPFS(cid);
          ...
          if (
            ...
            console.warn("IPFS metadata unavailable for this transfer — data may have been unpinned");
            this.emit("ipfsUnavailable", { cid });
            // Continue without IPFS metadata — the file transfer itself
            // still works via B2/WebTorrent, this only affects the
            // encrypted metadata overlay (title, creators, etc.)
          } else {
            image_src = IPFSdata.image;

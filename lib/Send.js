...
export class Send extends EventEmitter {
  ...
  async verify(uploadFiles, sha = null, id = null, key = null, idHash = null) {
    /* console.log(
      `verify(uploadFiles=${uploadFiles[0].name} sha=${sha} id=${id} key=${key}) idHash=${idHash})`
    ); */
    ...
    if (id) {
      ...
      //this._createRoom(Room);
      try {
        await this.room.verify(uploadFiles, sha, id, key, idHash);
      } catch (err) {
